...
public class BaseInstance implements ISubgraphInstance {
	...
	//removing final
	private  Long2ObjectMap<InstancePropertyMap> _vertexInstances;
	private  Long2ObjectMap<InstancePropertyMap> _edgeInstances;
	...
	// new methods added to decrease the rehasing time by initializing with size of edge list
	public void setEdgeInstanceSize(int size){
		
		if(_edgeInstances.size() == 0 ){ // if edgeInstance data is not filled
			
			System.out.println("Size changed");
			_edgeInstances = new Long2ObjectOpenHashMap<>(size);
		}
	}
	
	// new methods added to decrease the rrehasing time by initializing with size of vertex list
	public void setVertexInstanceSize(int size){
		
		if(_vertexInstances.size() == 0 ){ // if edgeInstance data is not filled
			_vertexInstances = new Long2ObjectOpenHashMap<>(size);
		}
	}
