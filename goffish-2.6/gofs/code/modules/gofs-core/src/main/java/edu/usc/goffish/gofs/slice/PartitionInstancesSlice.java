...
final class PartitionInstancesSlice implements ISlice {
	...
	public void addInstance(InstanceTuple tuple, ISubgraphInstance instance) {
        ...
        //changing the inital size to avoid rehashing
		Long2ObjectOpenHashMap<Object> values = new Long2ObjectOpenHashMap<>(propertyValues.size(), 1f);
	...
	boolean fillInstance(InstanceTuple tuple, SliceInstance instance) {
		...
		if(tuple.IsVertexProperty)
			instance.setVertexInstanceSize(propertyValues.size());
		else
			instance.setEdgeInstanceSize(propertyValues.size());
